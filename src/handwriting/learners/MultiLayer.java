...
public class MultiLayer extends PerceptronNet {
    ...
    protected void backpropagate(double[] inputs, double rate) {
        /* TODO: Calculate the backpropagated error for each hidden/output node pair.
           ...
           to store the errors.
         */
        for (int hiddenIndex = 0; hiddenIndex < hiddenToOutput.numInputNodes(); hiddenIndex++) {
            double error = 0.0;

            for (int out = 0; out < hiddenToOutput.numOutputNodes(); out++) {
                error += hiddenToOutput.error(out) * hiddenToOutput.weight(hiddenIndex, out) * gradient(inputToHidden.output(hiddenIndex));
            }
            inputToHidden.setError(hiddenIndex, error);
