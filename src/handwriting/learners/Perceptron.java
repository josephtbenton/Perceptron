...
public class Perceptron extends PerceptronNet {
    ...
    public void addToWeightDeltas(double[] inputs, double rate) {
        for (int out = 0; out < numOutputNodes(); out++) {
            for (int in = 0; in < numInputNodes(); in++) {
                deltas[in][out] += inputs[in] * errors[out] * rate * gradient(outputs[out]);
            }
            deltas[threshold()][out] -= errors[out] * rate * gradient(outputs[out]);
        }
    }
